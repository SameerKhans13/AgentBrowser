...
This document details the design and execution lifecycle of the Autonomous Web Agent.
...
### 2. Think (Decide)
The linearized representation is bundled with instructions and the latest viewport snapshot and sent to the **LLM**. The LLM evaluates the state and outputs a structured choice of action along with its strategic thought.
